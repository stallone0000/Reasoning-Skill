...
from distill_skill_cards import trace_from_item
...
async def process_one(
    ...
    *,
    ...
    if args.mock:
        summary = mock_summary(item.get("question", ""), trace, item.get("answer", ""))
        return {
            **item,
    ...
    for retry_attempt in range(args.max_retries + 1):
        ...
        _, summary = response_texts(payload)
        if not summary and retry_attempt < args.max_retries:
            await asyncio.sleep(min(20.0, 2.0**retry_attempt))
            continue

        usage = usage_counts(payload, prompt_text=prompt, response_text=summary)
        return {
            **item,
            "summary_model": args.model,
            "free_summary": summary,
            "summary_prompt_tokens": usage["prompt_tokens"],
            "summary_completion_tokens": usage["completion_tokens"],
            "summary_total_tokens": usage["total_tokens"],
            "status": "success" if summary else "error",
            "error": "" if summary else "empty summary",
            "retry_attempt": retry_attempt,
            "timestamp": now_iso(),
        }
...
async def run(args: argparse.Namespace) -> None:
    ...
    print(f"wrote {len(out)} free summaries -> {args.output_file}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize reasoning traces into compact free-form hints.")
    ...
    parser.add_argument("--mock", action="store_true", help="Run offline with deterministic toy summaries.")
