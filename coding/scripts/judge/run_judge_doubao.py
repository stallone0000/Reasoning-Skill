...
高性能多进程 judge pipeline for Doubao Seed 2.0 Pro 输出 (JSONL 格式)。

适配:
  - 输入格式: JSONL (每行一个 JSON 对象)
  - 输出字段: llm_output (而不是 gemini_outputs)
  - 支持 reasoning_content 格式 (已由 gen_doubao_seed.py 处理)
...
THOUGHT_END_RE = re.compile(
    ...
    r'|<\s*/\s*redacted_reasoning\s*>',  # doubao 格式
    re.IGNORECASE,
)
...
def main():
    ap = argparse.ArgumentParser(description="高性能多进程 Judge Pipeline (Doubao Seed 2.0 Pro)")
    ap.add_argument("--input", required=True, help="Doubao output JSONL file")
