# -*- coding: utf-8 -*-
"""
run_judge_qiniu_oss.py

高性能多进程 judge pipeline for Qiniu GPT-OSS-120B 输出 (JSONL 格式)。

适配:
  - 输入格式: JSONL (每行一个 JSON 对象)
  - 输出字段: llm_output (而不是 gemini_outputs)
  - 支持 reasoning_content 格式

核心优化:
  1. 使用 multiprocessing.Pool (fork) 替代 ThreadPoolExecutor
  2. 使用 imap_unordered 流式获取结果
  3. 每完成 1 条立即写入, 每 10 条 flush 一次
  4. 详细的实时进度和 ETA

用法:
  # 标准用法 (96 进程，与 gemini 保持一致)
  python3 run_judge_qiniu_oss.py \
    --input  qiniu_oss_cp_34799_v1.jsonl \
    --cases  nemotron_cp_cases_34799_v1.jsonl \
    --output judge_results_qiniu_oss.jsonl \
    --workers 96
    ...
    with path.open('r', encoding='utf-8') as f:
        for line in f:
            ...
            try:
                ...
                print(f"Warning: 跳过无效 JSON 行: {e}", file=sys.stderr)
                continue
...
def get_output_text(rec: dict) -> Optional[str]:
    """从 qiniu 格式记录中提取输出文本。"""
    text = rec.get(OUTPUT_KEY)
...
def _worker_judge_with_retry(rec: dict) -> dict:
    """
    对 JUDGE_ERROR 自动重试，避免短暂环境/资源抖动造成的假失败。
    """
...
def main():
    ap = argparse.ArgumentParser(description="高性能多进程 Judge Pipeline (Qiniu GPT-OSS-120B)")
    ap.add_argument("--input", required=True, help="Qiniu output JSONL file")
    ...
    ap.add_argument("--workers", type=int, default=96, help="Number of worker processes (default: 96)")
